    ...
    >
      <div className="p-4 flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-1.5 mb-1 md:mb-2">
            <span className={`size-1.5 rounded-full ${labelPrazo(data) === 'hoje' ? 'bg-sky-400 shadow-[0_0_5px_rgba(56,189,248,0.5)]' : 'bg-zinc-300'}`} />
            <span className={`text-[10px] md:text-[11px] font-black uppercase tracking-widest ${labelPrazo(data) === 'hoje' ? 'text-sky-400' : 'text-zinc-300'}`}>
              {labelPrazo(data)}
            </span>
          </div>
        ...
        <div className="flex flex-col items-end gap-1.5 shrink-0">
          ...
          {pendenciasCount > 0 && (
            <span className="text-[10px] md:text-[11px] font-bold px-2 py-0.5 rounded-md bg-[#450a0a] text-[#ef4444] border border-red-900/30">
              {pendenciasCount}p em aberto
            </span>
          )}
          {compact && (
      ...
      {ultimaVisita && (
        <div className="mx-4 mb-4 p-3.5 bg-black/30 rounded-xl border border-white/5">
          <div className="flex items-center gap-1.5 mb-1.5">
            <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.15em] text-sky-400">Última Visita</span>
          </div>
          <p className="text-zinc-200 text-[11px] md:text-[12px] leading-relaxed italic line-clamp-3">
            &quot;{ultimaVisita}&quot;
          </p>
        </div>
      )}
      ...
      {event.pendenciasLista && event.pendenciasLista.length > 0 && (
        <div className="mx-4 mb-5">
          <p className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.15em] text-zinc-200 mb-3">
            ...
            {event.pendenciasLista.map((p: { id: number; descricao: string; dataPrazo?: string }, i: number) => (
              <div key={i} className="flex items-center justify-between gap-3 group">
                ...
                <div className="flex items-center gap-2 min-w-0">
                  <div className="size-1 rounded-full bg-white group-hover:bg-sky-500 transition-colors" />
                  <p className="text-white text-[12px] font-medium truncate">{p.descricao}</p>
                </div>
                {p.dataPrazo && (
        ...
        <div className="px-4 pb-4">
          <Link 
            href={`/visitas/escolha?contratoId=${event.idContrato}&tipo=${encodeURIComponent(event.tipoVisita || '')}&modalidade=${encodeURIComponent(event.modalidade || '')}&dataHora=${encodeURIComponent(event.dataHora || '')}&agendadaId=${event.idVisita || ''}`} 
            className="block" 
