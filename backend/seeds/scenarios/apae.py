...
class APAEScenario(Scenario):
    ...
    def simulate_timeline(self, cliente, contrato, mode="realistic"):
        ...
        v_imersao = Visita(
            ...
            status="realizada",
            data_hora=data_relativa_dias(-10),
            duracao_estimada_minutos=480, # 8 horas de imersão
        ...
        if mode in ["realistic", "stress"]:
            v_reuniao_remota = Visita(
                ...
                id_contrato=contrato.id_contrato,
                status="realizada",
                data_hora=data_relativa_dias(-2),
            ...
            visitas.append(v_reuniao_remota)
        ...
        if mode == "stress":
            eventos.append(create_causal_event(v_imersao, "Atraso no Repasse", "Prefeitura atrasou repasse da subvenção, cliente em risco de fluxo de caixa.", impacto="Crítico Operacional", resolvido=False))
