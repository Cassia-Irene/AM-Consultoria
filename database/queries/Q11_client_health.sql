...
SELECT 
    ...
    -- Status Operacional (Derivado de Fatos)
    CASE 
        WHEN COALESCE(de.eventos_ativos, 0) > 0 OR COALESCE(dv.visitas_urgentes, 0) > 2 THEN 'emergência'
        WHEN COALESCE(dp.pendencias_atrasadas, 0) > 3 OR (COALESCE(dent.total_entregas, 0) > 0 AND (COALESCE(dent.entregas_concluidas, 0)::float / dent.total_entregas) < 0.2) THEN 'atenção'
        ELSE 'normal'
