    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-zinc-200 text-[10px] md:text-[11px] font-bold tabular-nums">
            {displayDate(visita.data_hora)}
          </span>
        </div>

        <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6">
          <p className="text-[10px] md:text-[11px] font-black uppercase tracking-widest text-zinc-300 mb-3">Relato de Campo</p>
      ...
      {/* RESULTADOS / IMPACTO */}
      {visita.resultados && (
        <section className="bg-emerald-950/10 border border-emerald-900 rounded-2xl p-6">
          <p className="text-[10px] font-black uppercase tracking-widest text-emerald-500 mb-3">Impacto / Resultados</p>
          <p className="text-emerald-100 text-sm italic leading-relaxed">&quot;{visita.resultados}&quot;</p>
        </section>
      )}
      ...
      <section className="grid grid-cols-2 gap-4">
        <div className="bg-zinc-900/30 border border-zinc-800/50 rounded-2xl p-4">
          <p className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-zinc-300 mb-2">Modalidade</p>
          <p className="text-zinc-300 text-xs md:text-sm font-bold capitalize">{visita.modalidade}</p>
        </div>
        <div className="bg-zinc-900/30 border border-zinc-800/50 rounded-2xl p-4">
          <p className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-zinc-300 mb-2">Duração Est.</p>
          <p className="text-zinc-200 text-xs md:text-sm font-bold">1h 30m</p>
        </div>
      </section>

      {/* AÇÕES RELACIONADAS (PENDÊNCIAS GERADAS) */}
      <section className="space-y-4">
        <p className="text-[10px] font-black uppercase tracking-widest text-sky-500 px-1">Ações Geradas nesta Visita</p>
        <div className="space-y-3">
          {/* Aqui idealmente filtraríamos pendências que tem contratoId e data_criacao próxima */}
          <div className="bg-zinc-900/20 border border-zinc-800 rounded-xl p-4 flex items-center justify-between group">
             <div className="flex items-center gap-3">
                <div className="size-1.5 rounded-full bg-emerald-500" />
                <p className="text-xs text-zinc-200 font-medium">Revisão de faturamento mensal</p>
             </div>
