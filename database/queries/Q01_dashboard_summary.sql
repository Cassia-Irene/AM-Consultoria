...
WITH KPIs AS (
    SELECT 
        (SELECT COUNT(*) FROM contratos WHERE data_fim IS NULL) as contratos_ativos,
        ...
        (SELECT COUNT(*) FROM faturamento_cliente WHERE NOT pago AND mes_ano < CURRENT_DATE) as inadimplencia_count,
